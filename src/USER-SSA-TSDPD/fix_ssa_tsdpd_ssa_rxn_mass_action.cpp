  ...
  Fix(lmp, narg, arg)
{
  ...
  if (atom->ssa_reaction_flag == 0) 
    error->all(FLERR,"Illegal number of reactants. Unable to call fix_ssa_tsdpd_ssa_rxn_mass_action.");
  ...
  if(num_reactants > atom->num_ssa_species) 
  ...
  if(num_products > atom->num_ssa_species) 
...
void FixSsaTsdpdSsaRxnMassAction::post_force(int vflag)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      ...
      // first zero out your product lines
      ...
      if(num_reactants==2){
        if(reactants[0] == reactants[1]){
            ...
            ssa_rxn_propensity[i][rxn_index] = k_rate/volume*C[i][reactants[0]]*C[i][reactants[1]];
            d_ssa_rxn_prop_d_c[i][rxn_index][reactants[0]] = k_rate/volume;
            d_ssa_rxn_prop_d_c[i][rxn_index][reactants[1]] = k_rate/volume;
      ...
      /*if(i==2074){
      for(int j=0;j<atom->num_ssa_species;j++){
        printf("ssa_stoich_matrix[%i][%i][%i] = %i\n",i,rxn_index,j,ssa_stoich_matrix[i][rxn_index][j]);
      }
      }*/
    }
