...
Atom::Atom(LAMMPS *lmp) : Pointers(lmp)
{
  ...
  solid_tag = NULL; // solid_tag = 1 if particle is solid, 0 if fluid
  fixed_tag = NULL; // fixed_tag = 1 if fixed particle, 0 if not (e.g., flexible solid)
  phi = NULL; // indicator function (boundary volume fraction)
  number_density = NULL;
  nw = NULL; // normal to boundary
  near_solid_neighbor = NULL; // stores a near solid neighbor of particle i
  v_weighted_solid = NULL; // weighted velocity of solid particles nearby
  a_weighted_solid = NULL; // weighted acceleration of solid particles nearby
  deviatoricTensor = NULL;  // deviatoric tensor
  ddeviatoricTensor = NULL; // differential of deviatoric tensor
  artificialStressTensor = NULL; // artificial stress tensor
  kernelCorrectionTensor = NULL; // kernel correction tensor
  ddx = ddv = NULL; // particle shifting arrays (Zhang et al., 2019)
    // -- additional variables (in case of SSA) --
  Cd = Qd = NULL; // concentration, flux (SSA)
  ...
  dfsp_D_matrix = NULL; // SSA diffusion matrix
  dfsp_D_diag = NULL; // SSA diffusion matrix diagonal
  dfsp_Diffusion_coeff = NULL; // SSA diffusion coefficients
  Pold = NULL;
  Pnew = NULL;
  Aaux = NULL;
  Baux = NULL;
...
Atom::~Atom()
{
  ...
  memory->destroy(C);
  memory->destroy(Q);
  memory->destroy(solid_tag);
  memory->destroy(fixed_tag);
  memory->destroy(phi);
  memory->destroy(number_density);
  memory->destroy(nw);
  memory->destroy(deviatoricTensor);
  memory->destroy(ddeviatoricTensor);
  memory->destroy(artificialStressTensor);
  memory->destroy(kernelCorrectionTensor);
  memory->destroy(ddx);
  memory->destroy(ddv);
  memory->destroy(Pold);
  memory->destroy(Pnew);
  memory->destroy(Aaux);
  memory->destroy(Baux);
  memory->destroy(APaux);
  memory->destroy(fP);
  memory->destroy(rhoI);
  memory->destroy(rhoAux1);
  memory->destroy(rhoAux2);
  memory->destroy(rhoAux3);
