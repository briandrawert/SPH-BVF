...
PairSsaTsdpdBvfFsi::PairSsaTsdpdBvfFsi(LAMMPS * lmp): Pair(lmp) {
  restartinfo = 0;
  first = 1;
  random = NULL;
}
...
PairSsaTsdpdBvfFsi::~PairSsaTsdpdBvfFsi() {
  if (allocated) {
...
void PairSsaTsdpdBvfFsi::init_style() {
...
/* ---------------------------------------------------------------------- */
void PairSsaTsdpdBvfFsi::compute(int eflag, int vflag) {
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	//else             fpair = imass * jmass * ( (fj/(rho[j]*rho[j])) - (fi/(rho[i]*rho[i])) ) * wfd;
        ...
        if (solid_tag[i] == 1) { // if i is a solid particle
          for (int m = 0; m < 3; m++) {
            for (int n = 0; n < 3; n++) {
              G0i = G0[itype] * (1.0 - 0.99 * C[i][0]);
              G0j = G0[jtype] * (1.0 - 0.99 * C[j][0]);
              ...
              ddeviatoricTensor[i][m][n] += 2.0 * ((2.0 * G0i * G0j) / (G0i + G0j + 1e-12)) * (strainTensor[m][n] - (1. / 3.) * kroneckerTensor[m][n] * strainTensor[m][n]) + deviatoricDotRotation + rotationDotDeviatoric;
            }
          }
        }
        ...
        if (solid_tag[i] == 1) { // if i is a solid particle
          for (int m = 0; m < 3; m++) {
            for (int n = 0; n < 3; n++) {
              ...
              if (totalStressi > 0.0) artificialStressTensor[i][m][n] = -0.1 * totalStressi / (rho[i] * rho[i]);
              else artificialStressTensor[i][m][n] = 0.0 * totalStressi / (rho[i] * rho[i]);
            }
          }
        ...
        if (solid_tag[j] == 1) { // if j is a solid particle
          for (int m = 0; m < 3; m++) {
            for (int n = 0; n < 3; n++) {
              ...
              if (totalStressj > 0.0) artificialStressTensor[j][m][n] = -0.1 * totalStressj / (rho[j] * rho[j]);
              else artificialStressTensor[j][m][n] = 0.0 * totalStressj / (rho[j] * rho[j]);
            }
          }
...
void PairSsaTsdpdBvfFsi::allocate() {
  allocated = 1;
  int n = atom -> ntypes;
...
void PairSsaTsdpdBvfFsi::settings(int narg, char ** arg) {
  if (narg != 0)
    error -> all(FLERR,
...
void PairSsaTsdpdBvfFsi::coeff(int narg, char ** arg) {
...
double PairSsaTsdpdBvfFsi::init_one(int i, int j) {
...
double PairSsaTsdpdBvfFsi::single(int i, int j, int itype, int jtype,
