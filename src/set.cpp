...
enum{TYPE,TYPE_FRACTION,MOLECULE,X,Y,Z,CHARGE,MASS,SHAPE,LENGTH,TRI,
     ...
     SMD_CONTACT_RADIUS,DPDTHETA,SSATSDPD_E,SSATSDPD_RHO,SSATSDPD_C,SSATSDPD_CD,
     SSATSDPD_CV,SSATSDPD_SOLIDTAG,INAME,DNAME,VX,VY,VZ,};
  ...
  while (iarg < narg) {
    ...
    if (strcmp(arg[iarg],"type") == 0) {
      ...
      if (strcmp(arg[iarg+2],"NULL") == 0) ivalue = 0;
        ...
        ivalue = force->inumeric(FLERR,arg[iarg+1]);
        ivalue2 = force->inumeric(FLERR,arg[iarg+2]);
      if (ivalue < 0) error->all(FLERR,"Illegal set command");
    ...
    } else if (strcmp(arg[iarg],"ssa_tsdpd/solid_tag") == 0) {
     if (iarg+3 > 5 || iarg+3 < 4) error->all(FLERR,"Illegal set command");
      else {
        ivalue = force->inumeric(FLERR,arg[iarg+1]);
        if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
        if (strcmp(arg[iarg+2],"NULL") == 0) ivalue2 = 0;
        if (strcmp(arg[iarg+2],"fixed") == 0) ivalue2 = 1;
        if (strcmp(arg[iarg+2],"free") == 0) ivalue2 = 0;
      }
     set(SSATSDPD_SOLIDTAG);
      iarg += 3;
...
void Set::set(int keyword)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    else if (keyword == SSATSDPD_SOLIDTAG) {
        if (ivalue < 0 || ivalue > 1) error->one(FLERR,"Invalid value in set solid_tag command. Solid tag must be 0 for fluid, and 1 for solid particles");
        else {
	    atom->solid_tag[i] = ivalue;
	    atom->fixed_tag[i] = ivalue2;
	  }
        }
