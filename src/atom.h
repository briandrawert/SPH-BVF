...
class Atom : protected Pointers {
 public:
  ...
  // USER-SSA-TSDPD package

  int *solid_tag,*near_solid_neighbor,*fixed_tag;
  double *phi;
  double **C,**Q,**nw,*number_density,**v_weighted_solid,**a_weighted_solid;
  double ***deviatoricTensor,***ddeviatoricTensor;
  double ***artificialStressTensor;
  double ***kernelCorrectionTensor;
  int num_sdpd_species;
  int num_sdpd_reactions;
  int num_ssa_species;
  int num_ssa_reactions;
  double **ddx,**ddv;
  // -- additional arrays (in case of SSA) --
  int **Cd, **Qd;   // added (Cd = SSA concentration, Qd = SSA flux of mass)
  ...
  double *dfsp_D_matrix, *dfsp_D_diag, *dfsp_Diffusion_coeff, *dfsp_a_i; //Diffusion matrix, diagonal, coefficients and propensities for SSA
  ...
  //USER-SSA-TSDPD
  int sdpd_flag;
  int ssa_diffusion_flag;
  int ssa_reaction_flag;
  int fixed_solid_flag;
