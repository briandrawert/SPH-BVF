...
PairSsaTsdpdBvfMechanics::PairSsaTsdpdBvfMechanics(LAMMPS * lmp): Pair(lmp) {
  restartinfo = 0;
  first = 1;
  random = NULL;
}
...
PairSsaTsdpdBvfMechanics::~PairSsaTsdpdBvfMechanics() {
  if (allocated) {
...
void PairSsaTsdpdBvfMechanics::compute(int eflag, int vflag) {
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	//velocity corr
        ddx[i][0] += pow(jmass/rho[j],2) * (v[j][0] - v[i][0]) * wf;	
        ddx[i][1] += pow(jmass/rho[j],2) * (v[j][1] - v[i][1]) * wf;	
        ddx[i][2] += pow(jmass/rho[j],2) * (v[j][2] - v[i][2]) * wf;	
  	  ...
  	  ddx[j][0] += pow(imass/rho[i],2) * (v[i][0] - v[j][0]) * wf;	
          ddx[j][1] += pow(imass/rho[i],2) * (v[i][1] - v[j][1]) * wf;	
          ddx[j][2] += pow(imass/rho[i],2) * (v[i][2] - v[j][2]) * wf;	
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        fpair = imass * jmass * ( (fj/(rho[j]*rho[j])) + (fi/(rho[i]*rho[i])) ) * wfd;
	...
	//else             fpair = imass * jmass * ( (fj/(rho[j]*rho[j])) - (fi/(rho[i]*rho[i])) ) * wfd;
        ...
        if (solid_tag[i] == 1) { // if i is a solid particle
          for (int m = 0; m < 3; m++) {
            for (int n = 0; n < 3; n++) {
              totalStressi = deviatoricTensor[i][m][n] - fabs(fi) * kroneckerTensor[m][n];
        ...
        if (solid_tag[j] == 1) { // if j is a solid particle
          for (int m = 0; m < 3; m++) {
            for (int n = 0; n < 3; n++) {
              totalStressj = deviatoricTensor[j][m][n] - fabs(fj) * kroneckerTensor[m][n];
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (solid_tag[i] == 0) { // if i is a fluid particle ...
          if (solid_tag[j] == 1) { // if j is a solid particle ...
            ...
            if (fixed_tag[j] == 0) { // if j is a free solid particle ...
	    ...
	    else{ // if j is a fixed solid particle
              v_weighted_solid[i][0] += 0.0;
              v_weighted_solid[i][1] += 0.0;
              v_weighted_solid[i][2] += 0.0;
              a_weighted_solid[i][0] += 0.0;
              a_weighted_solid[i][1] += 0.0;
              a_weighted_solid[i][2] += 0.0;
	    }
          }
        }
        ...
        if (newton_pair || j < nlocal) {
	      ...
	      if (fixed_tag[i] == 0) { // if i is a free solid particle ...
	      ...
	      else { // if i is a fixed solid particle ...
                v_weighted_solid[j][0] += 0.0;
                v_weighted_solid[j][1] += 0.0;
                v_weighted_solid[j][2] += 0.0;
                a_weighted_solid[j][0] += 0.0;
                a_weighted_solid[j][1] += 0.0;
                a_weighted_solid[j][2] += 0.0;
	      }
            }
          }
        }
...
void PairSsaTsdpdBvfMechanics::allocate() {
  allocated = 1;
  int n = atom -> ntypes;
...
void PairSsaTsdpdBvfMechanics::settings(int narg, char ** arg) {
  if (narg != 0)
    error -> all(FLERR,
...
void PairSsaTsdpdBvfMechanics::coeff(int narg, char ** arg) {
...
double PairSsaTsdpdBvfMechanics::init_one(int i, int j) {
...
double PairSsaTsdpdBvfMechanics::single(int i, int j, int itype, int jtype,
