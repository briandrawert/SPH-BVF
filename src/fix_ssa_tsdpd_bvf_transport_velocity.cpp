...
#include <stdio.h>
#include <string.h>
#include "fix_ssa_tsdpd_bvf_transport_velocity.h"
#include <math.h>
#include <stdlib.h>
#include <string.h>
#include "atom.h"
...
FixSsaTsdpdBvfTransportVelocity::FixSsaTsdpdBvfTransportVelocity(LAMMPS *lmp, int narg, char **arg) :
    Fix(lmp, narg, arg) {
...
int FixSsaTsdpdBvfTransportVelocity::setmask() {
    int mask = 0;
...
void FixSsaTsdpdBvfTransportVelocity::init() {
    dtv = update->dt;
    dtf = 0.5 * update->dt * force->ftm2v;
}
/* ---------------------------------------------------------------------- */

void FixSsaTsdpdBvfTransportVelocity::setup_pre_force(int vflag) {
  // set vest equal to v
  double ** v = atom -> v;
...
void FixSsaTsdpdBvfTransportVelocity::initial_integrate(int vflag) {
    ...
    if (mask[i] & groupbit) {
      ...
      if (atom -> fixed_tag[i] == 0) {
        ...
        // If solid particle
          ...
          // Compute half-step deviatoric stress tensor
          for (int m = 0; m < 3; m++) {
            for (int n = 0; n < 3; n++) {
      ...
      // If particles are fixed (velocity = 0, position = x0, acceleration = 0)
      else {
...
void FixSsaTsdpdBvfTransportVelocity::final_integrate() {
    ...
    if (mask[i] & groupbit) {
      ...
      if (atom -> fixed_tag[i] == 0) {
        ...
        // If solid particle
          ...
          // Compute final deviatoric stress tensor
          for (int m = 0; m < 3; m++) {
            for (int n = 0; n < 3; n++) {
              deviatoricTensor[i][m][n] += 0.5 * dtv * ddeviatoricTensor[i][m][n];
            }
...
void FixSsaTsdpdBvfTransportVelocity::reset_dt() {
    dtv = update->dt;
    dtf = 0.5 * update->dt * force->ftm2v;
}
