  ...
  Fix(lmp, narg, arg)
{
  ...
  boussinesq_sdpd_flag = gravity_flag = 0;

  //if (strcmp(arg[3],"boussinesq/ssa") == 0)  boussinesq_ssa_flag = 1;
  if (strcmp(arg[3],"boussinesq/sdpd") == 0)  boussinesq_sdpd_flag = 1;
  else if (strcmp(arg[3],"gravity") == 0)  gravity_flag = 1;
  else error->all(FLERR,"Illegal type of force in fix ssa_tsdpd/buoyancy command. Valid options: <boussinesq/sdpd> or <gravity>");
